...
function TotalRevenue()
{
	...
	try
	{
		...
		var val12 = myGetValue( "new_globalmonitorrevenue" );
		var val13 = myGetValue( "new_stratxrevenue" );
		...
		if( out1 != null )
			Sum13( "Total Revenue", val01, val02, val03, val04, val05, val06, val07, val08, val09, val10, val11, val12, val13, out1 );

		var out2 = Xrm.Page.getAttribute( "estimatedvalue" );	
		if( out2 != null )
			Sum13( "Estimated Revenue Value", val01, val02, val03, val04, val05, val06, val07, val08, val09, val10, val11, val12, val13, out2 ); 
	}
	catch( err )
	{
...
function TotalCost()
{
	...
	try
	{
		...
		var val8 = myGetValue( "new_globalmonitorcost" );
		var val9 = myGetValue( "new_stratxcosts" );
		var out1 = Xrm.Page.getAttribute( "cust_totalcost" );
		if( DebugModeOppII ) window.alert( "function TotalCost\nval1 " + val1 + "\nval2 " + val2 + "\nval3 " + val3 + "\nval4 " + val4 + "\nval5 " + val5 + "\nval6 " + val6 + "\nval7 " + val7  + "\nval8 " + val8 + "\nval9 " + val9 );
		
		if( out1 != null )
			Sum9( "Total Cost", val1, val2, val3, val4, val5, val6, val7, val8, val9, out1 );
	}
	catch( err )
	{
...
function Earned_Update( iSwitch )
{
	try
	{
		...
		switch( iSwitch )
		{
			...
			case 8:
				StratxEarned_Update();
				StratxEarnedGP_Update();
				StratxEarnedGPpct_Update();
				break;
			case 9:
				GlobalMonitorEarned_Update();
				GlobalMonitorEarnedGP_Update();
				GlobalMonitorEarnedGPpct_Update();
				break;
			default:
				break;
		}
