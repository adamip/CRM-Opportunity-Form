/*
	...
	Adam Ip																	2012-02-09
	showhideInformationTab() calls mySetFocus( "Topic" );
*/
...
function myFireEvent( elmt, strEvt )
{
	try
	{
		if( DebugModeOpp ) window.alert( "function myFireEvent()\nelmt\t" + elmt + "\nstrEvt\t" + strEvt );
	...
	{
		if( DebugModeOpp ) window.alert( "function myFireOnChangeEvent()\nerror code\t" + err + "\nerror name\t" + err.name + "\nerror message\t" + err.message );
	}
}
...
function showhideInformationTab()
{
	try
	{
		DebugModeOpp = DebugModeOpp && AmISoftwareDeveloper();
		if( DebugModeOpp ) window.alert( "function showhideInformationTab()" );

	/* Regardless of conditions, always show the following tabs */
		ShowOneTab( "General" );
		ShowOneTab( "Notes and Activities" );
		ShowOneTab( "Quotes" );
		ShowOneTab( "Preferences" );
		ShowOneTab( "Customer Requirements" );

		/* Regardless of conditions, always hide the following tabs */
		HideOneTab( "Product Line Item Details" );
		HideOneTab( "tab_Earned_GP_Pct" );

		/* always show this Section, regardless of situations */
		ShowHideOneTabSection( "Managed Services", true );

		if( AmIShowGeneralHidden() || AmISoftwareDeveloper() )
			ShowOneTab( "General Hidden" );
		else	
			HideOneTab( "General Hidden" );
...
function InitialPriceListOnCreate()
{
	...
	{
		if( DebugModeOpp ) window.alert( "function InitialPriceListOnCreate()\nerror code\t" + err + "\nerror name\t" + err.name + "\nerror message\t" + err.message );
	}
}
